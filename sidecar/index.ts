...
function handleMessage(message: SDKMessage): void {
  ...
  switch (message.type) {
    ...
    case "tool_result": {
      const result = message as { tool_use_id?: string; content?: unknown; is_error?: boolean };
      bus.emit({
        type: "tool_result",
        turnId,
    ...
    case "error": {
      const err = message as { type: string; error?: unknown };
      bus.emit({ type: "error", message: String(err.error) });
      break;
    }
  }
}
