...
public sealed class SidecarEventStream : BackgroundService
{
        ...
        lock (sync)
        {
            ...
            switch (evt.Type)
            {
                ...
                case "tool_call_started":
                    // Any file the agent touches (read/write/edit) becomes viewable in
                    // chat via /local-file, even outside the workspace — the operator
                    // saw the call. gate_request carries the same path pre-decision.
    ...
    // Pull a file path out of a tool call's input (Read/Write/Edit and friends use
    // file_path; a few use path/sourceFilePath/filePath).
